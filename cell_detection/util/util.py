...
def center_point(mask):
	...
	for cordinates in prop:
		...
		if not math.isnan(temp_center[0]) and not math.isnan(temp_center[1]):
			...
			cell_num += 1
	return np.clip(center_mask, a_min=0, a_max=1).astype(np.uint8), cell_num
...
def center_edge(mask, image):
	center_map, cell_num = center_point(mask) ##图片尺寸
	print("center num", len(center_map), cell_num)
	edge_map = draw_individual_edge(mask)
	# print('centerpoint', len(center_map))
	comb_mask=edge_map
	# comb_mask=center_map+edge_map
	comb_mask=np.clip(comb_mask, a_min=0, a_max=1)
	check_image=np.copy(image)
	comb_mask*=255
	check_image[:,:,1]=np.maximum(check_image[:,:,1], comb_mask) #方法:计算极值
