...
def cut_tiles(slide_path, tile_size, magnification, ratio, MIL_use):
    ...
    # env_path = os.path.join('../lmdb', tile_size, svs_name)
    # env = lmdb.open(index_path, map_size=int(1e9)) #lmdb初始化
    # env.close()
    # print(env, 'envbulid')#成功创建
    ...
    net_process(out_path, index_path, tile_size, MIL_use)
    ...
    #
    # print(txn.stat())
    print(txn.stat()['entries'], 'lmdblength')  #读取LMDB文件的样本数量
...
def get_tiles(slide_path, out_path, index_path, tile_size, magnification, ratio, color_normalization=False):
    ...
    # print(list(tqdm(pool.imap(partial_work, range(address[1])), total = int(address[1]), desc = '进程进度')))
    # print(range(address[1]))

    pool.map(partial_work, range(address[1]))  ##降低空间复杂度
...
def pool_process(row, pool_param):
    ...
    env = lmdb.open(index_path, map_size=int(1e9))
    # print(env, 'envopen')
    ...
    for col in range(address[0]):
        ...
        if flag and col != address[0] - 1 and row != address[1] - 1:
            ...
            db_key = str(str(col) + '_' + str(row))
            # # print(tile,'tileimg',db_key)
            # # print(source_imgage,'datasource')
            # key_byte = db_key.encode('ascii')
            # data = source_imgage
            # txn.put(key_byte, data)
            db_data = np.array(tile)
            txn.put(db_key.encode(), str(db_data).encode())
    ...
    txn.commit()
    env.close()
    # print(txn.get(db_key.encode()),'lmdbget')
    # txn = env.begin()
    # for key, value in txn.cursor():
    #     print(key, value)
    # env.close()
...
def inference(model, dataset, device, out_path, index_path):
    ...
    svs_index = open(index_path + '/' + 'svs_index.txt', 'w')
    neg_index = open(index_path + '/' + 'neg_index.txt', 'r')
    tum_index = open(index_path + '/' + 'tum_index.txt', 'w')
    ...
    for imgs, path in dataloader:
        ...
        with torch.no_grad():
            ...
            for log in batch_log:
                ...
                row = annotation[2]
                if cancer_type == 9:
                    tum_index.write(out_path + '/' + ' ' + str(col) + ' ' + str(row) + ' ' + '.jpg' + '\n')
                s_mes = out_path + '/' + ' ' + str(cancer_rate) + ' ' + str(col) + ' ' + \
                        str(row) + ' ' + str(cancer_type) + \
                        ' ' + str(rate1) + \
                        ' ' + str(rate2) + \
...
def net_process(out_path, index_path, tile_size, MIL_use):
    ...
    if tile_size == 224:
        if MIL_use:
            pretrained_dict = torch.load('./models/12_14/CRC9_resnext50_256_0.995_0.991_0.947.ckpt')
            model = models.resnext50_32x4d()
        else:
            print('unuse')
            pretrained_dict = torch.load('./models/12_14/CRC9_resnet18_1024_0.985_0.985_0.943.ckpt')  # resnet18 自写的效果不好
            model = models.resnet18()
        # pretrained_dict = torch.load('./models/resnet_process_224/best9classtensor_adam0.9985_dropout_32.ckpt') #resnet18 自写的
        # pretrained_dict = torch.load('./models/12_14/CRC9_resnext50_256_0.995_0.991_0.947.ckpt') #resnext50_ 模型体积差一倍
        # model = resnet224()
        # model = models.resnext50_32x4d()
        # model = models.resnet18()
