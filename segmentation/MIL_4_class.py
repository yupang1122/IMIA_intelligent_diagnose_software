...
def get_tiles(slide_path, out_path, index_path, tile_size, magnification, ratio, color_normalization=False):
    ...
    pool_param = [slide_path, tile_size, ratio, out_path, index_path, dz_level, address, target_max, target_stain_matrix]
...
def pool_process(row, pool_param):
    slide_name, tile_size, ratio, out_path, index_path, dz_level, address, target_max, target_stain_matrix = pool_param
...
def net_process(out_path, index_path, tile_size):
    ...
    if tile_size == 224 :
        pretrained_dict = torch.load('./models/MIL_4/train_19_loss_0.007893614841355085_acc_0.9973522121234882.ckpt')
        # model = resnet224()
